...
#define _SC_STREAM_MAX FOPEN_MAX
#endif
...
/* 	lua_pushnumber(L, sleep(seconds)); */
  ...
  double frac = fmod(secs, 1.0);
  secs = floor(secs);

  req.tv_sec = (time_t)secs;
  req.tv_nsec = (long)(frac*1000000000.0);

  res = nanosleep(&req, &rem);

  if(res == -1)
  {
    rems = (double)rem.tv_sec + ((double)rem.tv_nsec)/1000000000.0;    
    lua_pushnumber(L, res);
    lua_pushnumber(L, rems);
    return 2;
  }
  else
  { 
...
static int Pctime(lua_State *L)
{
	time_t z = (time_t)luaL_checknumber(L, 1);
  lua_pushstring(L, ctime(&z));
  return 1;
}
...
LUALIB_API int luaopen_libposix (lua_State *L)
{
