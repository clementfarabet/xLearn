...
function PyramidPacker:__init(scales, network)
   parent.__init(self)
   -- vars
...
function PyramidPacker:forward(input)
   ...
   if ((input:size(1) ~= self.dim_width) or
    (input:size(2) ~= self.dim_height)) then
      self.dim_width = input:size(1)
      self.dim_height = input:size(2)
      self.coordinates, self.max_width, self.max_height = 
	 getCoordinates({dim_width = self.dim_width, dim_height = self.dim_height, scales = self.scales, 
			 step_width = self.step_width, step_height = self.step_height})
   end
   
   if(input:size(3) ~= dim_z) then self.dimz = input:size(3) end
   self.output:resize(self.max_width,self. max_height, self.dimz)
   self.output:fill(0)
   ...
   for i = 1,#self.scales do
      local temp = self.output:narrow(1,self.coordinates[i][1],self.coordinates[i][5])
...
function PyramidPacker:backward(input, gradOutput)
   error('# ERROR: nn.PyramidPacker.backward() non implemented')
end
