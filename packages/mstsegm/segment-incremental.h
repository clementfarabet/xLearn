...
image<rgb> *segment_incrementalben(image<float> **img,
                                   int nfeats, int reset,
                                   float sigma, float c, int min_size,
                                   int dt, int method, THTensor *matches,
                                   int *num_ccs) {
  // get dims
  image<float> *im = img[0];
  int width = im->width();
  ...
  // extra matches (a match is a 'shortcut' between the previous and 
  // the current frame: it creates an edge with weight 0)
  int nb_matches = 0;
  if (matches != NULL) nb_matches = matches->size[2];
  ...
  if (img_prev != NULL) {
    ...
    if (method == 4) {
      ...
      // build graph with 4-connex
      edges = new edge[width*height*nslices*2 + width*height*(nslices-1) + nb_matches];
    ...
    // build graph with 8-connex
      edges = new edge[width*height*nslices*4 + width*height*(nslices-1) + nb_matches];
      for (int z = 0; z < nslices; z++) {
        for (int y = 0; y < height; y++) {
          for (int x = 0; x < width; x++) {
            if (x < width-1) {
    ...
    // add one edge for each match
    if (matches != NULL) {
      int x1, y1, x2, y2;

      for (int i=0; i<nb_matches; i++) {
        x1 = THTensor_get3d(matches, 0, 0, i);
        y1 = THTensor_get3d(matches, 1, 0, i);
        x2 = THTensor_get3d(matches, 0, 1, i);
        y2 = THTensor_get3d(matches, 1, 1, i);

        edges[num].a = (0 * height + y2) * width + x2;
        edges[num].b = (1 * height + y1) * width + x1;
