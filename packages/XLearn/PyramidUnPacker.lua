...
function PyramidUnPacker:forward(input, coordinates)
   ...
   for i = 1, self.coordinates:size(1) do
      ...
      --print('start_x = ', start_x,'start_y = ', start_y, 'width = ', width, 'height = ', height)
      
      local temp = input:narrow(1, start_x, width)
...
function PyramidUnPacker:backward(input, gradOutput)
   error('# ERROR: nn.PyramidUnPacker.backward() non implemented')
end
