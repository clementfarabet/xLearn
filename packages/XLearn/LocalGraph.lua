local LocalGraph, parent = torch.class('nn.LocalGraph', 'nn.Module')
...
function LocalGraph:__init(...)
   parent.__init(self)

   toolBox.unpack_class(self, {...}, 'nn.LocalGraph',  help_desc,
   ...
   _ = (self.connex == 4) or xerror('4 is the only connexity supported, for now',
                                    'nn.LocalGraph',self.usage)
   self.dist = ((self.dist == 'euclid') and 0) or ((self.dist == 'cosine') and 1)
       or xerror('euclid is the only distance supported, for now','nn.LocalGraph',self.usage)
...
-- define fprop in C
LocalGraph.forward_c = inline.load [[
      // get all params
...
-- define bprop in C
LocalGraph.backward_c = inline.load [[
      // get all params
      const void* torch_Tensor_id = luaT_checktypename2id(L, "torch.Tensor");
      THTensor *input = luaT_checkudata(L, 2, torch_Tensor_id);
