...
static int jpeg_size(lua_State *L)
{
...
static int jpeg_load(lua_State *L)
{
  /* This struct contains the JPEG decompression parameters and pointers to
   * working space (which is allocated as needed by the JPEG library).
   */
  ...
  tensor = THTensor_newWithSize3d(cinfo.output_width, cinfo.output_height, cinfo.output_components);
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
    ...
    {
      double *tdata = THTensor_dataPtr3d(tensor, 0, cinfo.output_scanline-1, k);
      for(i = 0; i < cinfo.output_width; i++)
        tdata[i] = (double)buffer[0][cinfo.output_components*i+k];
    }
  }
