...
static int sync_get_both(void ***data, uint32_t *timestamp, int index)
{
  buffer_ring_t *dbuf = &kinects[index]->depth;
  buffer_ring_t *vbuf = &kinects[index]->video;

  // lock depth buffer
  pthread_mutex_lock(&dbuf->lock);
  while (!dbuf->valid)
    pthread_cond_wait(&dbuf->cb_cond, &dbuf->lock);

  // lock video buffer
  pthread_mutex_lock(&vbuf->lock);
  while (!vbuf->valid)
    pthread_cond_wait(&vbuf->cb_cond, &vbuf->lock);

  void *temp_buf;

  // get video data
  temp_buf = vbuf->bufs[0];
  data[0] = vbuf->bufs[0] = vbuf->bufs[1];
  vbuf->bufs[1] = temp_buf;
  vbuf->valid = 0;
  *timestamp = vbuf->timestamp;
  pthread_mutex_unlock(&vbuf->lock);

  // get depth data
  temp_buf = dbuf->bufs[0];
...
int freenect_sync_get_rgbdepth(void ***array, uint32_t *timestamp, int index, 
                               freenect_depth_format dfmt, freenect_video_format vfmt)
{
	if (index < 0 || index >= MAX_KINECTS) {
		printf("Error: Invalid index [%d]\n", index);
		return -1;
	...
	if (!thread_running || !kinects[index] 
            || kinects[index]->depth.fmt != dfmt 
            || kinects[index]->video.fmt != vfmt)
          if (setup_kinect(index, dfmt, 1) || setup_kinect(index, vfmt, 0))
            return -1;
	sync_get_both(array, timestamp, index);
	return 0;
}
